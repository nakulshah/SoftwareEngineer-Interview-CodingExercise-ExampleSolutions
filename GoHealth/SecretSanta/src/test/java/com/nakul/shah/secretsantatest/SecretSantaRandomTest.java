...
import com.nakul.shah.secretsantaimpl.SecretSantaRandomImpl;
 ...
 * @author V506617
 */
public class SecretSantaRandomTest 
{
    private final String[] participants = new String[] { "Kyle", "Kenny", "Eric", "Stan",
                                        "Stewie", "Brian" };
    ...
    private SecretSanta secretSantaRandom;
    
    
    public SecretSantaRandomTest() 
    {
        secretSantaRandom = new SecretSantaRandomImpl();
    }
    ...
    {
        for (int stress = 0; stress <= 0; stress++)
        {
            String[] assignmentsRandom = secretSantaRandom.generateAssignments(participants);
            
            System.out.println("Assignment: " + Arrays.toString(assignmentsRandom));
            
            for(int i = 0; i < assignmentsRandom.length; i++)
            {
                if(assignmentsRandom[i].equalsIgnoreCase(participants[i]))
                    fail("Voilation of Rule: no person must be assigned to himself/herself. "
                            + "Violation by " + assignmentsRandom[i]);
            }
            
            if(!noDuplicates(assignmentsRandom))
            {
                fail("Assignment has duplicates " + Arrays.toString(assignmentsRandom));
                return;
            }
        }
    ...
    {
        final String[] assignmentsRandom = secretSantaRandom.generateAssignments(participants);
        
        for(String assignment : assignmentsRandom)
        {
            ...
            if(!fromPassed)
                ...
                assertTrue(noDuplicates(assignmentsRandom));
        }
    }
    ...
    {
       final String[] assignments = secretSantaRandom.generateAssignments(participants);
    ...
    public void nullParticipants() 
    {
        try
        {
            secretSantaRandom.generateAssignments(nullParticipants);
        }
        catch(Exception ex)
        {
    ...
    public void nullParticipantName() 
    {
        try
        {
            secretSantaRandom.generateAssignments(nullNameParticipants);
        }
        catch(Exception ex)
        {
    ...
    public void singleParticipant() 
    {
        try
        {
            secretSantaRandom.generateAssignments(singleParticipant);
        }
        catch(Exception ex)
        {
